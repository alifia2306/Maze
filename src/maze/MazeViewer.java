...
/**
 * The <code>MazeViewer</code> provides visualization panel for the maze.
 * Contains the driver for calling various maze solving and maze formation
 * algorithms.
 * 
...
public class MazeViewer extends JPanel {
	...
	public void init(String[] args) {
		...
		setSize(maze.getCols() * size + 2 * pad, maze.getRows() * size + 2
				* pad);
		...
		for (int j = 0; j < maze.getCols(); j++) {
			for (int i = 0; i < maze.getRows(); i++) {
				...
				if (cell.visited()) {
					...
					g2D.fill(new Rectangle2D.Double(x + 5, y + 5, size - 10,
							size - 10));
				...
				if (cell == maze.getStartCell()) {
					...
					g2D.fill(new Rectangle2D.Double(x + 5, y + 5, size / 2,
							size / 2));
					g2D.setColor(Color.black);
				}
	...
	private boolean checkArgs(String[] args) {
		if (!(args[0].equals("dfs") || args[0].equals("bfs") || args[0]
				.equals("random")))
			return false;
		return true;
	}
