...
/**
 * The <code>Maze</code> class contains the maze struture, which is just an
 * array of <code>MazeCell</code>s. Also contains the algorithms for generating
...
public class Maze {
	...
	public synchronized void makeKruskalMaze() {
		...
		while (num < maze.length * maze[0].length - 1) {
			...
			// If there's a wall to be knocked down. We do union find for the
			// two cells.
	...
	/**
	 * Solves the maze by depth first search. starts at the start cell
	 * maze[0][0] and stops when dfs discovers the end cell. Here cells are
	 * marked yellow when they are discovered(visited) and marked grey when they
	 * are fully examined.
	 */
	...
	/**
	 * Solves the maze by breadth first search. starts at the start cell and
	 * stops when bfs discovers the end cell. Here cells are marked yellow when
	 * they are discovered(visited) and marked grey when they are fully
	 * examined.
	 */
